...
def can_change_status(user: Dict[str, Any], current_status: str, new_status: str) -> bool:
    """
    사용자의 상태 변경 권한 확인
    ...
    # 진행 -> 완료/이슈/취소
    if current_status == "IN_PROGRESS" and new_status in ["COMPLETE", "ISSUE", "CANCEL"]:
        return True
...
def can_edit_order(user: Dict[str, Any], order_data: Dict[str, Any]) -> bool:
    ...
    # 본인이 락을 소유해야 함
    if order_data.get("isLocked", False) and order_data.get("updatedBy") != user.get("user_id"):
        return False
...
def get_status_options(user: Dict[str, Any], current_status: str) -> List[Dict[str, str]]:
    """
    상태 변경 옵션 목록 반환 (템플릿용)
